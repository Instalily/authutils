...
import logging
from typing import Dict, Any, Union # Import Union for type hinting

# Configure logging for this module
logger = logging.getLogger(__name__)

from .registry import BackendAuthProviderRegistry
from .models import ProviderTypeEnum, GrantTypeEnum, AccessTokenRequestInternal, RefreshTokenRequestInternal, TokenRequest
from .utils.pkce import (
    generate_code_verifier,
    generate_code_challenge,
    generate_csrf_state,
)
from .storage.pkce import PKCEStorage
from .core.exchange import exchange_authorization_code, exchange_refresh_token
...
async def initiate_auth_flow(
    ...
    Raises:
        ValueError: If the provider is unsupported, not found, or if there's
                    an issue generating parameters or building the URL.
        RuntimeError: If core components or storage were not initialized.
    ...
    try:
        ...
        raise ValueError(f"Unsupported provider: {provider_name_str}") from e
    except Exception as e:
        logger.error(f"Auth Flow Error: Retrieving auth provider for {provider_name_str}: {e}", exc_info=True)
        raise RuntimeError(f"Backend error configuring provider: {provider_name_str}") from e
    ...
    try:
        ...
        raise RuntimeError("Backend error generating PKCE parameters.") from e
    ...
    try:
        ...
        raise RuntimeError("Backend error generating state parameter.") from e
    ...
    try:
        ...
        raise RuntimeError("Backend error storing authentication state.") from e
    ...
    try:
        ...
        raise RuntimeError(f"Backend error building authorization URL for {provider_name_str}") from e
...
async def process_token_exchange(
    ...
    Raises:
        ValueError: If the request body is invalid or missing fields (handled by Pydantic),
                    or if the underlying exchange function raises a ValueError.
        RuntimeError: If core components or storage were not initialized,
                      or for unexpected backend errors.
    ...
    if registry is None or PKCEStorage is None or AccessTokenRequestInternal is None or RefreshTokenRequestInternal is None or exchange_authorization_code is None or exchange_refresh_token is None:
         ...
         raise RuntimeError("Backend authentication service is not fully configured.")
    ...
    if grant_type == GrantTypeEnum.AUTHORIZATION_CODE: # Use the Enum member
        ...
        try:
            ...
            if provider_type != stored_provider_type_enum: # Compare Enum members directly
                 ...
                 # For security, you might want to raise a ValueError here.
                 # raise ValueError("Provider mismatch in request.")

        except ValueError as e: # Catch ValueErrors from storage (e.g., invalid state)
             raise e # Re-raise the ValueError
        except Exception as e:
            logger.error(f"Auth Flow Error: Retrieving PKCE state for state {state[:10]}...: {e}", exc_info=True)
            raise RuntimeError("Backend error retrieving authentication state.") from e
        ...
        try:
            ...
            logger.debug(f"Auth Flow: Retrieved provider '{stored_provider_type_enum.value}' for authorization code exchange.")
             ...
             raise RuntimeError(f"Backend configuration error: Stored provider '{stored_provider_type_enum.value}' not registered.") from e
        except Exception as e:
             logger.error(f"Auth Flow Error: Getting auth provider for stored type {stored_provider_type_enum.value}: {e}", exc_info=True)
             raise RuntimeError(f"Backend error getting provider: {stored_provider_type_enum.value}") from e
        ...
        try:
        ...
        except ValueError as e: # Catch errors raised by the exchange function
           logger.warning(f"Auth Flow Error: Core authorization code exchange failed: {e}")
           raise ValueError(f"Token exchange failed: {e}") from e # Re-raise the ValueError

        except RuntimeError as e: # Catch unexpected errors from the exchange function
           logger.error(f"Auth Flow Unexpected Error: Core authorization code exchange: {e}", exc_info=True)
           raise RuntimeError("An internal error occurred during token exchange.") from e

        except Exception as e: # Catch any other unexpected errors
            logger.error(f"Auth Flow Unexpected Error: Processing authorization code exchange: {e}", exc_info=True)
            raise RuntimeError("An unexpected error occurred during authorization code exchange.") from e
         ...
         try:
             ...
             raise ValueError(f"Unsupported provider for refresh: {provider_type.value}") from e
         except Exception as e:
              logger.error(f"Auth Flow Error: Getting auth provider for refresh {provider_type.value}: {e}", exc_info=True)
              raise RuntimeError(f"Backend error getting provider for refresh: {provider_type.value}") from e
         ...
         try:
            ...
            raise ValueError(f"Token refresh failed: {e}") from e # Re-raise the ValueError

         except RuntimeError as e: # Catch unexpected errors from the exchange function
            logger.error(f"Auth Flow Unexpected Error: Core refresh token exchange: {e}", exc_info=True)
            raise RuntimeError("An internal error occurred during token refresh.") from e

         except Exception as e: # Catch any other unexpected errors
             logger.error(f"Auth Flow Unexpected Error: Processing refresh token exchange: {e}", exc_info=True)
             raise RuntimeError("An unexpected error occurred during refresh token exchange.") from e
